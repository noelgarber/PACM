...
def add_matrix_weights(slim_length, matrices_dict, position_weights):
    '''
    ...
    for key, df in matrices_dict.items():
        for i, weight in enumerate(position_weights):
            position = "#" + str(i+1)
            df[position] = df[position] * weight
...
def score_aa_seq(sequence, weighted_matrices, slim_length, dens_df = None, df_row_index = None, add_residue_cols = False):
    ...
    flanking_left_weighted_matrices = np.array([weighted_matrices.get(key) for key in flanking_left_keys])
    flanking_right_weighted_matrices = np.array([weighted_matrices.get(key) for key in flanking_right_keys])

    # Compute score values
    flanking_left_scores = np.array([flanking_left_weighted_matrices[index].at[current_residues, "#" + str(left_positions[index])] for index in np.arange(slim_length)])
    flanking_right_scores = np.array([flanking_right_weighted_matrices[index].at[current_residues, "#" + str(right_positions[index])] for index in np.arange(slim_length)])
    total_position_scores = flanking_left_scores + flanking_right_scores

    # Get the total score
    output_total_score = np.sum(total_position_scores)

    # Optionally add columns for each residue in the sequence, for easy searching in the output
    if add_residue_cols:
        residue_col_names = np.char.add("Residue_", np.arange(1, slim_length + 1).astype(str))
        dens_df.at[df_row_index, residue_col_names] = sequence_array
...
def diagnostic_value(score_threshold, dataframe, significance_col = "One_Passes", score_col = "SLiM_Score",
    ...
    sensitivity = TP_count/(TP_count+FN_count) if (TP_count+FN_count) > 0 else np.nan
    specificity = TN_count/(TN_count+FP_count) if (TN_count+FP_count) > 0 else np.nan
    ppv = TP_count/(TP_count+FP_count) if (TP_count+FP_count) > 0 else np.nan
    npv = TN_count/(TN_count+FN_count) if (TN_count+FN_count) > 0 else np.nan
...
def apply_threshold(input_df, sig_col = "One_Passes", score_col = "SLiM_Score", range_count = 100,
    ...
    for current_score in score_range_series:
        _, _, ppv, npv, fdr_val, for_val = diagnostic_value(score_threshold = current_score, dataframe = output_df,
...
def make_pairwise_matrices(dens_df, percentiles_dict = None, slim_length = None, minimum_members = None,
    ...
    if optimize_weights:
        ...
        best_fdr, best_for, best_score_threshold = 0, 0, 0
        ...
        for weights_array in expanded_weights_array:
            ...
            current_weighted_matrices_dict = add_matrix_weights(slim_length = slim_length, matrices_dict = matrices_dict,
                                                                position_weights = weights_list)
            ...
            current_best_score, current_best_fdr, current_best_for = apply_threshold(current_dens_df, sig_col = significance_col,
            ...
            if current_best_fdr > best_fdr:
                ...
                dens_df = current_dens_df

                print(f"\t\tNew record set for FDR={best_fdr} and FOR={best_for} with the weights {best_weights}") if verbose else None

        if best_weights is not None:
            print(f"\tOptimal weights for pairwise matrices: {best_weights}")
