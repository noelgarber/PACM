...
def predict_chunk(motifs_ids_tuple, topology_trim_begin, topology_trim_end, topological_domains, sequences,
    ...
    for i, (motif_seq, uniprot_swissprot_id) in enumerate(zip(motif_seqs, uniprot_ids)):
        print(i, motif_seq, uniprot_swissprot_id)
        domain_types = ""
        domain_descriptions = ""
                    ...
                    if trimmed_begin != -1:
                        ...
                        for domain_tuple in current_topological_domains:
                            ...
                            if motif_within:
                                ...
                                domain_types = topological_domain_type
                                ...
                                if len(domain_types) == 0:
                                    ...
                                    domain_types = f"{domain_types};{topological_domain_type}"
                                    domain_descriptions = f"{domain_descriptions_vals};{description}"

        domain_types_vals.append(domain_types)
        domain_descriptions_vals.append(domain_descriptions)
...
def predict_topology(data_df, motif_cols, predictor_params = predictor_params):
    ...
    for motif_col in motif_cols:
        ...
        for i in range(0, len(data_df), chunk_size):
            ...
            chunk_domain_types_vals, chunk_domain_descriptions_vals = predict_chunk(chunk_tuple,
                                                                                    topology_trim_begin,
                                                                                    topology_trim_end,
                                                                                    topological_domains,
                                                                                    sequences, verbose)
